...
def get_enabled_server_names(config: dict[str, Any]) -> list[str]:
    return [
        name
...
def is_probably_write_tool(tool_name: str) -> bool:
...
def filter_registry_for_mode(
    registry: dict[str, RegisteredTool],
    *,
    allow_writes: bool,
) -> tuple[dict[str, RegisteredTool], list[str]]:
    if allow_writes:
        return dict(registry), []

    allowed: dict[str, RegisteredTool] = {}
    blocked: list[str] = []
    for name, tool in registry.items():
        if is_probably_write_tool(name):
            blocked.append(name)
            continue
        allowed[name] = tool
...
async def connect_servers(
    ...
    if selected_servers:
        ...
        server_names = get_enabled_server_names(config)
        if not server_names:
            raise ValueError(
